...
def main():
    ...
    aleo = get_file_data("data/aleo.private.txt")
    logger.info(f"found {len(aleo)} aleo")
    ...
    if len(aleo) != len(tokens):
        raise Exception(f"Amount of aleo must be equals to amount of tokens")
    ...
    for idx, token in enumerate(tokens):
        try:
            ...
            response = send_msg_wrapped(message=f"/sendcredits {aleo_address} 50")
